# modified from https://github.com/ma-xu/pointMLP-pytorch
...
class Model(nn.Module):
    ...
    def forward(self, data):
        x = to_dense_batch(data.pos, data.batch)[0]
        xyz = x
        x = x.permute(0, 2, 1)
        # xyz = x.permute(0, 2, 1)  # fg: replaced with above, because in riginal implementation the permute also occurs in train_epoch, canceling each other
        batch_size, _, _ = x.size()
...
def pointMLP(points=1024, num_classes=40, **kwargs) -> Model:
...
def pointMLPElite(points, num_classes=40, **kwargs) -> Model:
