...
# python ph_classify.py --dataset mcb --root data/mcb_dataset/ --flaml-classifier lgbm --time-budget 600 --stats-file ./flood_mcb.yaml
def main():
    ...
    for k in split_ids:
        ...
        labs_trn, labs_tst, labs_val = [
            np.array(itemgetter(*idx)(labs), dtype=np.int64)
        ...
        automl = AutoML()
        assert args.metric_to_use in ["accuracy", "macro_f1"], "Metric not supported"
        settings = {
        ...
        avg_trn_balanced_accuracy_score = np.mean(
            trn_tracker["balanced_accuracy_score"]
        )
        avg_tst_balanced_accuracy_score = np.mean(
            tst_tracker["balanced_accuracy_score"]
        )
        std_trn_balanced_accuracy_score = np.std(trn_tracker["balanced_accuracy_score"])
        std_tst_balanced_accuracy_score = np.std(tst_tracker["balanced_accuracy_score"])
